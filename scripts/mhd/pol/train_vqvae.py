...
def main(seed: int, log_level: int):    
    ...
    train_images = xa.open_dataarray(train_fpath).sel(pol=['q', 'u']).transpose('batch', ..., 'pol').values.astype(np.float32)
    ...
    # Setup checkpointing
    checkpoint_root = f"/oasis/scratch/comet/bthorne/temp_project/began_scratch/mhd/mhd_pol/checkpoints/embeddings{num_embeddings:04d}"
    checkpoint_name = f"pol-vqvae-embed{num_embeddings:04d}"
    save_prefix = os.path.join(checkpoint_root, checkpoint_name)

    # Saved model path
    saved_model_path = f"/oasis/scratch/comet/bthorne/temp_project/began_scratch/mhd/mhd_pol/pol-vqvae-{num_embeddings:04d}"
    ...
    @tf.function(input_signature=[tf.TensorSpec([None, 256, 256, channels])])
    def inference(x):
