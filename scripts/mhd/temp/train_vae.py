...
def main(root_dir: Path, cfg_path: Path, seed: int, log_level: int):    
    ...
    data_cfg, train_cfg, loss_config, architecture_cfg, name = load_cfg(cfg_path)
    ...
    training_samples_path = data_dir / name.with_suffix('.cdf')
    ...
    train_images = xa.open_dataarray(train_fpath).transpose('batch', ..., 'pol').values.astype(np.float32)
    dset = tf.data.Dataset.from_tensor_slices(train_images)
    dset = dset.shuffle(train_images.shape[0])
    dset = dset.map(rotate, num_parallel_calls=4)
    dset = dset.map(flip, num_parallel_calls=4)
    dset = dset.batch(train_cfg['batch_size'])

    test_fpath = data_dir / data_cfg['test_file']
    test_images = xa.open_dataarray(test_fpath).transpose('batch', ..., 'pol').values.astype(np.float32)
    ...
    data = np.zeros((train_cfg['epochs'], 9, 256, 256, 1), dtype=np.float32)
    samples = xa.DataArray(data, coords=coords, dims=dims)
    z_batch = tf.random.normal(shape=[9, architecture_cfg['lat_dim']])
    ...
    for epoch in range(train_cfg['epochs']):
        ...
        for train_x in dset: 
            ...
            compute_apply_gradients(model, train_x, optimizer, beta_vae=loss_config['beta_vae'])

        # generate samples after training for an epoch
        samples[epoch] = model.decode(z_batch)

        # after every 10 epochs save summary statistics
        if epoch % 10 == 0:
            loss = tf.keras.metrics.Mean()
            for test_x in test_dataset:
                loss(began.vae.compute_loss(model, test_x, beta_vae=loss_config['beta_vae'], loss_type=loss_config['loss_type']))
            elbo = - loss.result()
            _logger.info(f"ELBO at epoch {epoch:05d} is {elbo:.03f}")

            #with summary_writer.as_default():
            #    tf.summary.scalar('elbo', elbo, step=epoch)
            facet_grid = samples.sel(epoch=epoch, pol='t').plot(x='x', y='y', col='batch', col_wrap=3, cmap='cividis', robust=True)
            facet_grid.fig.savefig(plot_dir / f"sample_epoch{epoch:04d}.png")
    model.save_weights(str(model_path))
    samples.to_netcdf(training_samples_path)
